...
<div class="max-w-7xl mx-auto pb-28">
    ...
    {{-- Flash messages --}}
    @if(session('success'))
    <div class="flex items-center gap-3 px-4 py-3 mb-6 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 text-sm font-medium" id="flash-ok">
        <span class="material-symbols-outlined text-base">check_circle</span>
        {{ session('success') }}
        <button onclick="document.getElementById('flash-ok').remove()" class="ml-auto text-emerald-500 hover:text-emerald-700">
            <span class="material-symbols-outlined text-base">close</span>
        </button>
    </div>
    @endif
    @if(session('error'))
    <div class="flex items-center gap-3 px-4 py-3 mb-6 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm font-medium">
...
window.saveThresholds = function() {
    // Also keep localStorage in sync so equipment page JS picks up new thresholds
...
db.ref('.info/connected').on('value', snap => {
    ...
    if (dot) dot.className = `w-3 h-3 rounded-full ${live ? 'bg-emerald-500 animate-pulse' : 'bg-amber-400 animate-pulse'}`;
    if (txt) txt.textContent = live ? 'Connected — Receiving live data' : 'Reconnecting to Firebase…';
});

db.ref("{{ $device?->firebase_path ?? 'drybox' }}").on("value", snapshot => {
    const data = snapshot.val();
    if (!data) return;
    const now  = new Date().toLocaleTimeString();
    document.getElementById('snap-hum').textContent    = parseFloat(data.humidity    ?? 0).toFixed(1) + ' %';
    document.getElementById('snap-temp').textContent   = parseFloat(data.temperature ?? 0).toFixed(1) + ' °C';
