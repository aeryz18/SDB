...
@section('title', 'Analytics & Trends — Demo | DryBox AI')
...
<div class="max-w-7xl mx-auto space-y-grid-gutter">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="flex items-center gap-3 self-start md:self-auto">
            <div class="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-outline-variant rounded-xl text-sm">
                ...
                <span class="font-medium text-xs text-emerald-600">Simulated</span>
            </div>
    ...
    {{-- ── Live KPI Tiles ──────────────────────────────────────── --}}
    <section class="grid grid-cols-2 md:grid-cols-4 gap-4" id="tour-kpi-tiles">
    ...
    {{-- ── Main Chart + Summary ────────────────────────────────── --}}
    <section class="grid grid-cols-1 lg:grid-cols-12 gap-grid-gutter" id="tour-analytics-chart">
        ...
        <div class="lg:col-span-8 bg-white border border-outline-variant rounded-xl p-md flex flex-col min-h-[420px]">
            <div class="flex justify-between items-center mb-5">
                <div>
                    ...
                    <p class="font-body-sm text-body-sm text-on-surface-variant">Humidity & Temperature — simulated data</p>
                </div>
                <span class="flex items-center gap-2 text-xs text-violet-600 font-bold uppercase tracking-wider bg-violet-50 px-3 py-1.5 rounded-full border border-violet-200">
                    <div class="w-2 h-2 rounded-full bg-violet-500 animate-pulse"></div> Demo
                </span>
            </div>
        ...
        <div class="lg:col-span-4 bg-tertiary-container text-on-tertiary-container rounded-xl p-md border border-tertiary shadow-lg flex flex-col justify-between overflow-hidden relative">
            <div class="relative z-10 space-y-4">
                <div>
                    ...
                    <p class="font-body-sm text-body-sm text-white/60">Simulated — updates every 2 seconds</p>
                </div>
    ...
    {{-- ── Event Log ────────────────────────────────────────────── --}}
    <div class="bg-white border border-outline-variant rounded-xl overflow-hidden" id="tour-event-log">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody id="analytics-log" class="divide-y divide-slate-100">
                    <tr>
                        <td colspan="4" class="px-md py-8 text-center text-sm text-slate-400">Generating simulated data…</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
  ...
  window.exportCSV = function() {
    ...
    a.download = `drybox-demo-analytics-${Date.now()}.csv`;
    a.click();
  };
  ...
  function simulateAnalytics() {
    simTemp += (Math.random() - 0.48) * 0.5;
    simHum  += (Math.random() - 0.42) * 1.2;
    simTemp  = Math.max(23, Math.min(35, simTemp));
    simHum   = Math.max(22, Math.min(52, simHum));

    const time = new Date().toLocaleTimeString();
    const hum  = parseFloat(simHum.toFixed(1));
    const temp = parseFloat(simTemp.toFixed(1));

    // Snapshot panel
    document.getElementById('snap-hum').innerHTML    = `${hum}<span class="text-sm">%</span>`;
    document.getElementById('snap-temp').innerHTML   = `${temp}<span class="text-sm">°C</span>`;
    document.getElementById('snap-time').textContent = time;

    const isCrit = hum > CRIT_THRESH;
    const isWarn = !isCrit && hum > WARN_THRESH;
    const stat   = isCrit ? 'Critical' : isWarn ? 'Warning' : 'Safe';
      ...
      <tr class="hover:bg-slate-50 transition-colors">
        ...
        <td class="px-md py-3">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm">Hum ${hum}% · Temp ${temp}°C</span>
          </div>
        </td>
  ...
  for (let i = 0; i < 6; i++) {
    const fH = 27 + Math.random() * 5;
    const fT = 25 + Math.random() * 3;
    const t  = new Date(Date.now() - (6 - i) * 2000).toLocaleTimeString();
    analyticsChart.data.labels.push(t);
    analyticsChart.data.datasets[0].data.push(parseFloat(fH.toFixed(1)));
    analyticsChart.data.datasets[1].data.push(parseFloat(fT.toFixed(1)));
    humReadings.push(fH); tempReadings.push(fT); readingCount++;
  }
  analyticsChart.update();
  updateKPIs();
