...
class SendMonthlyReport implements ShouldQueue
{
    ...
    public function handle(GmailApiSender $sender, FungusRisk $fungusRisk): void
    {
        ...
        $riskResult = $fungusRisk->evaluate($readings, $device->settings);

        $tempReadings = $readings->whereNotNull('temperature');
        $humReadings  = $readings->whereNotNull('humidity');
        $doorEvents   = $riskResult['stats']['door_open_events'] ?? 0;
