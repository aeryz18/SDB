...
**Incognito** — an AI-powered smart camera dry box for photographers and videographers (formerly branded "DryBox AI" / "AI-Powered Smart Camera Dry Box"), built on a Laravel 13 IoT monitoring dashboard for smart dry storage units. The system has a dual data flow: the browser reads Firebase directly for live sensor values, while a Laravel scheduler polls Firebase server-side every minute to persist history, evaluate alert rules, and send Gmail notifications.
...
| `alerts` | Alert log — `device_id`, `type` enum (`humidity_warn`, `humidity_crit`, `temp`, `silica_due`, `silica_drift`, `silica_upcoming`, `tamper`), `message`, `value`, `emailed_at`, `resolved_at`; composite index on `(device_id, type, resolved_at)`. `humidity_warn`/`humidity_crit` are generated by `AlertEvaluator` from `warn_humidity`/`crit_humidity` (crit takes priority over warn), same cooldown/email pattern as `temp`/`tamper`. (`fungus` was a historical enum value for a retired feature — fully removed, including deleting any existing rows, since the feature was deliberately dropped with no trace wanted, not just phased out.) |
...
- **`AlertEvaluator`** — `evaluate(Device, Reading): array` checks humidity (`warn_humidity`/`crit_humidity`, crit takes priority over warn), temperature (`temp_min`/`temp_max`), and door tamper. Checks cooldown via `alerts` table.
...
Loaded from CDN (`cdn.tailwindcss.com`) and configured via an inline `<script id="tailwind-config">` in both layout files — **not** via `tailwind.config.js` or Vite. The `@tailwindcss/vite` package is present but unused. Style changes must be applied to the inline config in **both** `layouts/app.blade.php` and `layouts/demo.blade.php` (and `layouts/setup.blade.php`, which carries a smaller copy).

**Incognito identity.** The brand is a camera-gear instrument, not a generic SaaS template. Dark mode reads as the device faceplate (Case Black `#101413`, Graphite panels, hairline Seams); light mode reads as the paper spec sheet (warm Desiccant Paper `#f5f4ef` ground, white cards). The brand accent is a custom `silica` orange scale (`silica-50`…`silica-950`, brand value `silica-500 #ff7a29` / `silica-600 #c2410c` — burnt silica) defined in every page's inline Tailwind config; it is the **only** brand accent (buttons, links, active nav, brand marks, the humidity chart line). The Material Design 3 color tokens (`primary`, `secondary`, `surface-*`, etc.) are also tinted to this warm neutral/orange family — `primary` is `#c2410c` so the many `bg-primary text-white` buttons across the app stay AA-compliant without per-view edits. Never use Tailwind's stock `blue-*`/`indigo-*`/`green-*` classes for brand accents — use `silica-*` (or the M3 tokens). Green/amber/red (`emerald-*`/`amber-*`/`red-*`) are reserved for **sensor status only** (dry/safe, warning, critical) — never repurpose them as decoration. Temperature chart lines/legend use neutral `slate-400`/grey, not orange (orange is humidity's color). Purple stays the "protection armed" status color; violet stays the "demo mode" badge color — leave both alone. Standalone pages (`welcome`, `auth/login`, `auth/register`, `offline`) each carry their own one-line `silica` config since they don't share a layout; `welcome`/`login`/`register` are the canonical design reference — copy their token values rather than reinventing them.

**Light/dark mode.** `darkMode: "class"` with the class set on `<html>` (`light`/`dark`). A pre-paint script in each layout `<head>` reads `localStorage.theme` (falling back to `prefers-color-scheme`) to avoid a flash; a `#theme-toggle` button in the topbar flips the class, persists the choice, and dispatches a `themechange` window event. Views are written **light-first** — dark mode comes from centralized `html.dark .{utility}` CSS overrides in the layout `<style>` blocks (covering `bg-white`, `slate-*`, M3 tokens, and the status tint families). When adding new UI, stick to the utility classes already covered by those overrides rather than adding per-element `dark:` variants. Chart.js charts are re-colored (ticks/grid) by a layout script listening for `themechange`. The landing and auth pages are intentionally fixed dark-glass (no toggle) — they're the faceplate look at all times, not theme-aware.

Design tokens: Material Design 3 color names as custom Tailwind classes, **Bricolage Grotesque** for display/headlines, **IBM Plex Sans** for body copy, **IBM Plex Mono** for data readouts and uppercase micro-labels (`label-caps`/`data-num` font tokens — this is what makes dashboard stat cards read as instrument type). Icons: Google Material Symbols Outlined from CDN. Brand icon: `camera_indoor`. Every text/background pairing must clear WCAG AA (4.5:1) — stock Tailwind `slate-300/400/500` fail against both the light paper background and dark card surfaces, so `layouts/app.blade.php`, `layouts/demo.blade.php`, and `layouts/setup.blade.php` each carry `html:not(.dark) .text-slate-300/400/500` overrides that darken them; don't remove these when touching the style blocks.
